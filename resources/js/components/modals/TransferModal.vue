        ...
        <div class="w-full max-w-md bg-surface-elevated border border-surface-border rounded-2xl shadow-xl overflow-hidden flex flex-col">
            ...
            <div class="px-6 py-4 border-b border-surface-border-subtle flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <ArrowRightLeft class="w-5 h-5 text-text-muted" />
            ...
            <div class="p-6 flex flex-col gap-4">
                ...
                <div v-if="availableDestinations.length === 0" class="p-4 rounded-xl bg-surface-canvas text-center text-xs text-text-muted">
                ...
                <div v-else class="space-y-2 max-h-60 overflow-y-auto">
                    <button
                        ...
                        type="button"
                        class="w-full p-3 rounded-xl bg-surface-canvas hover:bg-surface-elevated border border-surface-border-subtle hover:border-surface-border flex items-center justify-between transition cursor-pointer"
                    >
                        <div class="flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full bg-status-available"></span>
                            <span class="text-sm font-semibold text-text-primary">{{ dest.name }}</span>
                            <span v-if="dest.is_vip" class="text-xs uppercase font-semibold text-status-warning bg-status-warning/10 px-1.5 py-0.5 rounded-full border border-status-warning/40">VIP</span>
                        </div>
