<?php

namespace App\Services\Tv\Drivers;

use App\Models\Station;
use App\Models\StationAudit;
use App\Services\Tv\Contracts\TvDeviceDriverInterface;
use App\Services\Tv\TvStatus;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log;

class RokuTvDriver implements TvDeviceDriverInterface
{
    protected const ROKU_PORT = 8060;

    protected const WOL_PORT = 9;

    public function turnOn(Station $station): bool
    {
        $sentWol = false;
        if ($station->tv_mac_address) {
            $sentWol = $this->sendWakeOnLan($station->tv_mac_address, $station->tv_ip_address);
        }

        if ($station->tv_ip_address) {
            try {
                Http::timeout(2)->post("http://{$station->tv_ip_address}:".self::ROKU_PORT.'/keypress/PowerOn');
            } catch (\Throwable $e) {
                // Ignore timeout if TV was sleeping
            }
        }

        StationAudit::create([
            'station_id' => $station->id,
            'game_session_id' => $station->activeSession?->id,
            'event_type' => 'tv_wake_sent',
            'physical_state' => $station->tv_physical_state,
            'expected_state' => $station->current_state,
            'details' => ['driver' => 'roku', 'ip' => $station->tv_ip_address],
        ]);

        return true;
    }

    public function turnOff(Station $station): bool
    {
        if (! $station->tv_ip_address) {
            Log::warning("RokuTvDriver: Station #{$station->station_number} has no TV IP address.");

            return false;
        }

        try {
            $response = Http::timeout(2)->post("http://{$station->tv_ip_address}:".self::ROKU_PORT.'/keypress/PowerOff');
            $sent = $response->successful();
        } catch (\Throwable $e) {
            $sent = false;
        }

        StationAudit::create([
            'station_id' => $station->id,
            'game_session_id' => $station->activeSession?->id,
            'event_type' => 'manual_tv_off',
            'physical_state' => $station->tv_physical_state,
            'expected_state' => $station->current_state,
            'details' => ['driver' => 'roku', 'ip' => $station->tv_ip_address, 'success' => $sent],
        ]);

        return $sent;
    }

    public function pollStatus(Station $station): TvStatus
    {
        if (! $station->tv_ip_address) {
            return TvStatus::UNREACHABLE;
        }

        try {
            $response = Http::timeout(1.5)->get("http://{$station->tv_ip_address}:".self::ROKU_PORT.'/query/device-info');
            if ($response->successful()) {
                $body = $response->body();
                if (str_contains($body, '<power-mode>PowerOn</power-mode>')) {
                    return TvStatus::SCREEN_ON;
                }

                return TvStatus::STANDBY;
            }
        } catch (\Throwable $e) {
            return TvStatus::STANDBY;
        }

        return TvStatus::STANDBY;
    }

    protected function sendWakeOnLan(string $macAddress, ?string $ipAddress = null): bool
    {
        $cleanMac = str_replace([':', '-', ' '], '', $macAddress);
        if (strlen($cleanMac) !== 12) {
            return false;
        }

        $macBin = pack('H*', $cleanMac);
        $magicPacket = str_repeat(chr(0xFF), 6).str_repeat($macBin, 16);

        $targetIp = $ipAddress ?: '255.255.255.255';

        $socket = @socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);
        if (! $socket) {
            return false;
        }

        @socket_set_option($socket, SOL_SOCKET, SO_BROADCAST, 1);
        $sent = @socket_sendto($socket, $magicPacket, strlen($magicPacket), 0, $targetIp, self::WOL_PORT);
        @socket_close($socket);

        return $sent !== false;
    }
}
